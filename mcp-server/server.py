...
# Cache native Memoire handles and serialize access per database. The Python
# binding wraps a single FFI handle, so requests for the same DB must not call it
# concurrently.
_memoire_instances: Dict[str, Memoire] = {}
_memoire_locks: Dict[str, threading.Lock] = {}
...
def get_memoire(ctx: Context) -> Memoire:
    ...
    with _instance_lock:
        if db_path not in _memoire_instances:
            logger.info("event=memoire_init db_path=%s", db_path)
            _memoire_instances[db_path] = Memoire(db_path)
            _memoire_locks[db_path] = threading.Lock()
    return _memoire_instances[db_path]

def get_memoire_lock(ctx: Context) -> threading.Lock:
    """Return the per-database lock, creating the Memoire handle if needed."""
    db_path = get_db_path(ctx)
    get_memoire(ctx)
    return _memoire_locks[db_path]

@mcp.tool()
def memoire_remember(content: str, ctx: Context) -> dict:
    """
    ...
    try:
        chunks = run_memoire(ctx, lambda memoire: memoire.remember(content))
...
@mcp.tool()
def memoire_recall(query: str, ctx: Context, top_k: int = 5, include_shadow: bool = False) -> dict:
    """
    ...
    try:
        results = run_memoire(ctx, lambda memoire: memoire.recall(query, top_k=top_k))
        final_results = []
...
@mcp.tool()
def memoire_reinforce(memory_id: int, agent_output: str, task_succeeded: bool, ctx: Context) -> dict:
    """
    ...
    try:
        reinforced = run_memoire(
            ctx,
            lambda memoire: memoire.reinforce_if_used(memory_id, agent_output, task_succeeded),
        )
        return ok_response(memory_id=memory_id, reinforced=reinforced)
    except Exception as e:
        logger.exception("event=memoire_reinforce_failed memory_id=%s", memory_id)
        return error_response("reinforce_failed", str(e), memory_id=memory_id)

@mcp.tool()
def memoire_penalize(memory_ids: List[int], failure_severity: float, ctx: Context) -> dict:
    """
    ...
    try:
        results = run_memoire(
            ctx,
            lambda memoire: memoire.penalize_if_used(memory_ids, failure_severity),
        )
...
@mcp.tool()
def memoire_batch_feedback(memory_ids: List[int], task_succeeded: bool, task_output: str, ctx: Context) -> dict:
    """
...
@mcp.tool()
def memoire_resolve_conflicts(memory_id: int, ctx: Context) -> dict:
    """
    ...
    try:
        run_memoire(ctx, lambda memoire: memoire.resolve_contradictions(memory_id))
        return ok_response(memory_id=memory_id, resolved=True)
    except Exception as e:
        logger.exception("event=memoire_resolve_conflicts_failed memory_id=%s", memory_id)
        return error_response("resolve_conflicts_failed", str(e), memory_id=memory_id)

@mcp.tool()
def memoire_forget(memory_id: int, ctx: Context) -> dict:
    """
    ...
    try:
        success = run_memoire(ctx, lambda memoire: memoire.forget(memory_id))
...
@mcp.tool()
def memoire_count(ctx: Context) -> dict:
    """
    Total number of stored memory chunks.
    """
    try:
        count = run_memoire(ctx, lambda memoire: memoire.count())
...
@mcp.tool()
def memoire_clear(ctx: Context) -> dict:
    """
    Erase ALL memories. This cannot be undone.
    """
    try:
        run_memoire(ctx, lambda memoire: memoire.clear())
